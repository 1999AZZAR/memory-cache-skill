...
metadata: {"openclaw":{"requires":{"env":["REDIS_URL"]},"install":[{"id":"node","kind":"exec","command":"scripts/cache ping"}]}}
---
...
3. On first run, `scripts/cache` initializes a venv and installs dependencies.

## Core Workflows

### 1. Key Operations
Set, get, or delete data in the cache. All keys must use the `mema:` prefix.
- **Usage**: `bash $WORKSPACE/skills/memory-cache/scripts/cache set mema:<category>:<name> <value> [--ttl N]`
- **Usage**: `bash $WORKSPACE/skills/memory-cache/scripts/cache get mema:<category>:<name>`

### 2. Search & Scan
Safe listing of keys using the Redis SCAN command.
- **Usage**: `bash $WORKSPACE/skills/memory-cache/scripts/cache scan [pattern]`
